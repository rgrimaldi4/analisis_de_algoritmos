...
La sucesión de Fibonacci es una secuencia de números donde cada valor se obtiene sumando los dos valores anteriores. Los primeros valores de la sucesión son: **0, 1, 1, 2, 3, 5, 8, 13, 21, ...**
...
El problema consiste en calcular los valores de Fibonacci,  utilizando diferentes formas para resolver el mismo problema.
...
La segunda versión utiliza recursión con **memoización**. Esta consiste en almacenar los valores de Fibonacci que ya fueron calculados. Antes de calcular **F(n)**, el algoritmo verifica si el resultado se encuentra almacenado:
...
Si el valor ya existe, se retorna directamente y no se vuelve a realizar el cálculo. Si todavía no ha sido calculado, se calcula y posteriormente se almacena para poder utilizarlo nuevamente.
...
Cada una de estas llamadas puede generar nuevas llamadas recursivas hasta alcanzar los casos base. Debido a que varios valores se calculan repetidamente, el número de llamadas aumenta de forma exponencial conforme aumenta n.

Por ello, esta estrategia tienen una complejidad temporal de O(2^n)

### Fibonacci con memoización

La versión con memoización almacena cada resultado calculado.

Los valores **F(0), F(1), F(2), ..., F(n)** se calculan y se almacenan para evitar volver a realizar el mismo cálculo.

Por esta razón, el crecimiento se reduce a **O(n)**
...
Los valores comienzan en **n = 10** y aumentan de **2 en 2** hasta **n = 100**

Para cada valor de n se registran **Número de pasos** y **Tiempo de ejecución**.
...
Para ambas versiones se considera como un **paso** cada vez que se realiza una llamada a la función **fibo()**. El contador se incrementa mediante **pasos++;**, por lo tanto, el número de pasos permite observar la cantidad de llamadas realizadas por cada versión.
