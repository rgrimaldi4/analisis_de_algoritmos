...
void agregarAlInicio(Tnodo**lista, int dato){
    Tnodo*nuevo = NULL; 
...
void leer_archivo(int n, int **R){ //matriz cuadrada
    FILE *archivo = fopen("g_dirigido.csv", "r");
    for(int i = 0; i < n; i++){
...
void print_path(int g, int s, int v, int*pi){
    if(v==s){
        ...
        printf("-no se puede llegar de %d a %d\n", s,v);
    }else{
...
void DFS_visit(int g, int u, int*color, int*d, int*pi, int*f, int **matAdj, Tnodo **listaTopo){
    time_step++;
    d[u] = time_step;
    color[u] = gris;
    ...
    for(int v=0; v<g; v++){
        if(matAdj[u][v] == 1){
            if( color[v] == blanco ){
                pi[v] = u;
                DFS_visit(g,v, color, d, pi, f, matAdj, listaTopo);
            }
        }
    }
    ...
    agregarAlInicio(listaTopo, u);
}

void DFS(int g, int **matAdj, Tnodo **listaTopo){
    ...
    for(int i=0; i<g; i++){
        if(color[i] == blanco){
            DFS_visit(g, i, color,d,pi,f, matAdj, listaTopo);
        }
    }
